...
class TestSuite:
    def __init__(self,testsuite_id, testsuite_name, details, project_name):
        ...
        self.details = details #配置信息，IP，端口，协议
        self.project_name = project_name

    #获取测试套件包含所有用例的ID
    def get_testcases_id_for_testuite(self):
    ...
    def run_testsuite(self,http):
        ...
        for testcase_id in testcases_id_list_for_testsuit[:]:
            testcase_info = mytestlink.getTestCase(testcase_id)

            """
            构造测试用例对象，具体参数：
            名称，步骤，是否启用，然后放到project_name中
            """
            # 将以下信息记录到测试用例表（测试数据）中，调用logger.info
            ...
            testplan = '无计划'
            data = (
            executed_history_id, testcase_id, testcase_name, self.testsuite_name, testplan, self.project_name, 'Block',
            '')
            ...
            # 记录运行时间接收以时间元组，并返回以可读字符串表示的当地时间，格式由参数format决定
            ...
            sql_update = 'UPDATE ' + testcase_report_tb + ' SET runresult=\"%s\", runtime=\"%s\"' \
                                                          ...
                                                          (testcase_run_result, run_time, executed_history_id,
