...
class TestCase:
    def __init__(self, testcase_id, testcase_name, steps,
                 active_status, testproject):
    ...
    def run_testcase(self, http, testplan):
        ...
        for step in self.steps:
            ...
            data = (
            executed_history_id, self.testcase_id, self.testcase_name, testplan, self.testproject, step_id, step_number,
            '', protocol_all, host_all, port_all,
            ...
            try:
                ...
                logger.error('步骤【%s】信息填写错误：%s，停止执行用例【id=%s，name=%s】'%
            ...
            if step_run_result[0] == 'Error':
                ...
                sql_update = 'UPDATE ' + case_step_report_tb + ' SET runresult=\"%s\",reason=\"%s\", protocol_method=\"%s\", runtime=\"%s\"' \
                                                               ...
                                                               (step_run_result[0], fail_or_error_reason,
                                                                protocol_method, run_time, str(executed_history_id),
